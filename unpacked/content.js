	...
	toDownload.forEach(function (item, index) {
		if (item.getContent) {
			item.getContent(function (body, encode) {
				...
				var newURL = resolveURLToPath(item.url).path;
	...
	if (zip) {
		...
		getAllToDownloadContent(toDownload, function (result) {
			zip.createWriter(new zip.BlobWriter(), function (blobWriter) {
				addItemsToZipWriter(blobWriter,result,function(){
...
function addItemsToZipWriter(blobWriter, items, callback) {
	...
	if (item) {
		let resolvedContent = (item.encoding === 'base64') ? new zip.Data64URIReader('data:text/plain;base64,' + item.content) : new zip.TextReader(item.content);
